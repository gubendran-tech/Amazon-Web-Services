...
Description of available options:
  ...
  --help       Displays detailed usage information.
    ...
    Support pid monitoring. Customized 
  --monitor-pid=VALUE       Report if pid is not running
...
my $argv_size = @ARGV;
my $monitor_pid;
my $apache_pid_path = "/var/run/httpd/httpd.pid";
my $tomcat_pid_path = "/opt/apache-tomcat-8.0.32/bin/catalina.pid";
...
# check for empty values in provided arguments
if (defined($monitor_pid) && length($monitor_pid) == 0) {
  exit_with_error("Value (Service) of monitor pid is not specified.");
}
...
if ($monitor_pid) {
   my $pid = 0;
   if ($monitor_pid =~ m/^apache/gi || $monitor_pid =~ m/^httpd/gi) {
       foreach my $line (split('\n', `/bin/cat $apache_pid_path`)) {
          if($line =~ /(\d+)/) {
              $pid = $1;
          }
        }
        add_metric('MonitorApachePID', 'None', $pid);
   } 
   if ($monitor_pid =~ m/^tomcat/i) {
        foreach my $line (split('\n', `/bin/cat $tomcat_pid_path`)) {
            if($line =~ /(\d+)/) {
                $pid = $1;
             }
         }
         add_metric('MonitorTomcatPID', 'None', $pid);
   }
}

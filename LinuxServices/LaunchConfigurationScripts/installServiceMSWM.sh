...
intelligenceServer="10.0.20.6 msis.staging.mysck.internal"
downloadFiles="/opt/"
...
trap cleanup KILL TERM INT QUIT SIGKILL SIGINT
{
    ...
    /usr/local/bin/aws s3 cp s3://kb-sck-softwares/tomcat-8-installer-conf/index.html ${downloadFiles}
    ...
    /usr/local/bin/aws s3 cp s3://kb-sck-softwares/MicroStrategyWebAndMobile-10.3-wars/stg-AdminServers.xml ${downloadFiles}
    /usr/local/bin/aws s3 cp s3://kb-sck-softwares/MicroStrategyWebAndMobile-10.3-wars/sys_defaults.properties ${downloadFiles}
    cp stg-AdminServers.xml /opt/${apacheTomcat}/webapps/MicroStrategy/WEB-INF/xml/AdminServers.xml  
    cp sys_defaults.properties /opt/${apacheTomcat}/webapps/MicroStrategy/WEB-INF/xml/sys_defaults.properties
    chown tomcat.tomcat /opt/${apacheTomcat}/webapps/MicroStrategy/WEB-INF/xml/AdminServers.xml 
    chown tomcat.tomcat /opt/${apacheTomcat}/webapps/MicroStrategy/WEB-INF/xml/sys_defaults.properties

    mv stg-AdminServers.xml /opt/${apacheTomcat}/webapps/MicroStrategyMobile/WEB-INF/xml/AdminServers.xml  
    mv sys_defaults.properties /opt/${apacheTomcat}/webapps/MicroStrategyMobile/WEB-INF/xml/sys_defaults.properties
    chown tomcat.tomcat /opt/${apacheTomcat}/webapps/MicroStrategyMobile/WEB-INF/xml/AdminServers.xml 
    ...
    /usr/local/bin/aws s3 cp s3://kb-sck-softwares/httpd-conf/index.html ${downloadFiles}
    ...
    /usr/local/bin/aws aws ec2 monitor-instances --instance-ids ${instanceId}
...
#echo "Completed at $(date)" | mail -s "$HOSTNAME - MSTR Web & Mobile installed successfully in Instance: ${instanceId}" 
cat $tmpLog >> $log
cleanup 0
exit 0
